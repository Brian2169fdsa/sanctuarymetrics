...
| `SHAREPOINT` | Header copy for the SharePoint usage tab. Unlike every other object here, the tab's *figures* are not in `data.js` — it fetches `sharepoint-usage.json` from the site root at runtime and renders the same page shape with em-dash tiles when that file is absent. See SHAREPOINT.md |
